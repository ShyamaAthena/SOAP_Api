...
public class Main {
    ...
    static final String USERNAME ="shyama@athenahealth.com";
    static final String PASSWORD ="mithinmohan@270183TZZYd1OaxUAiOng6gLUZnp7";
      ...
      try {
        ...
        for (int i=0; i< saveResults.length; i++) {
          if (saveResults[i].isSuccess()) {
            ...
            for (int j=0; j< errors.length; j++) { System.out.println("ERROR creating record: " + errors[j].getMessage()); } } } } catch (Exception e) { e.printStackTrace(); } } 
    // updates the 5 newly created Leads 
    private static void updateLeads() { System.out.println("Update the 5 new test leads..."); Lead[] records = new Lead[5]; try { QueryResult queryResults = connection.query("SELECT Id, FirstName, LastName, Company FROM Lead ORDER BY CreatedDate DESC LIMIT 5"); if (queryResults.getSize() > 0) {
          for (int i=0;i<5;i++){
            // cast the SObject to a strongly-typed Lead
            Lead l = (Lead)queryResults.getRecords()[i];
        ...
        for (int i=0; i< saveResults.length; i++) {
          if (saveResults[i].isSuccess()) {
            ...
            Error[] errors = saveResults[i].getErrors();
            for (int j=0; j< errors.length; j++) { System.out.println("ERROR updating record: " + errors[j].getMessage()); } } } } catch (Exception e) { e.printStackTrace(); } } 
    // delete the 2 newly created Leads 
    private static void deleteLeads() { System.out.println("Deleting the 2 new test Leads..."); String[] ids = new String[2]; try { QueryResult queryResults = connection.query("SELECT Id, Name FROM Lead ORDER BY CreatedDate DESC LIMIT 2"); if (queryResults.getSize() > 0) {
